...
PR #251 run `31570720168` exposed an unrelated i18n CSV smoke race: the trace showed Playwright resolving and setting a hidden file input while the restored page state was still replacing that control, so no `/system/i18n/import` request reached the browser network log or backend. The smoke now follows the visible Import button and the resulting file chooser, preserving the POST, persisted-value, and error-download assertions. The focused test passed once and then three consecutive times locally. Exact-commit remote gates and immutable v0.10.17 publication remain pending.
