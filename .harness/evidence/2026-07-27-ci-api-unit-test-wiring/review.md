...
- Two PR-review findings were valid: discovery was top-level only, and ESM output could not load a bare directory import that resolved to `index.ts`. The runner now traverses nested suites and rewrites each ESM relative import from its resolved module path; a nested ESM suite exercises both paths.
