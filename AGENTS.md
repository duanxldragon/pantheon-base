...
5. **测试同步**：凡是改动页面路由、组件结构、接口契约、权限判断、i18n key、选择器、导入导出入口、seed 数据或 smoke 覆盖范围，必须在同一轮改动里同步更新对应测试、脚本、fixture、门禁或截图基线；禁止把“代码已变、测试后补”留到回归阶段才发现。
6. **测试先行**：提供代码变更时，必须同步给出验证方式、测试思路或脚本。
7. **文档同步**：如果改动影响模块边界、接口、菜单、权限、i18n、数据库，必须同步更新文档。

## 6.1 烟测数据口令约束（新增）

- **如果用户明确说“烟测保留数据”，默认按“保留 smoke fixture”执行。** 已支持的套件必须使用 `PANTHEON_SMOKE_PRESERVE_FIXTURES=1` 或等价 preserve 脚本运行，结束后不额外清理。
- **如果用户明确说“清理烟测数据”，默认直接执行 cleanup。** 未指定范围时执行全量清理；指定 IAM、组织、配置等范围时执行对应 cleanup 脚本。
- **新增或修改会生成可见烟测数据的用例时，必须在同一轮改动里同步接入 preserve/cleanup 规则。** 不能出现“测试能造数据，但保留/清理口令不生效”的漂移。
- **没有用户明确口令时，保持默认行为：烟测结束自动清理。**
