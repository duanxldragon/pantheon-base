...
const artifactDir = join(process.cwd(), '..', 'tmp', 'cmdb-qa');
...
test.describe('CMDB Host Management', () => {
  ...
  test('host list page loads with table', async ({ page }) => {
    ...
    await page.screenshot({ path: join(artifactDir, 'cmdb-host-list.png'), fullPage: true });
  ...
  test('host detail loads by ID', async ({ page }) => {
    ...
    await expect(page.locator('.page-container')).toBeVisible();
    await page.screenshot({ path: join(artifactDir, 'cmdb-host-detail.png'), fullPage: true });
  });
  ...
  test('group list page loads', async ({ page }) => {
    ...
    await page.screenshot({ path: join(artifactDir, 'cmdb-group-list.png'), fullPage: true });
    expect(consoleErrors).toEqual([]);
  });
});
