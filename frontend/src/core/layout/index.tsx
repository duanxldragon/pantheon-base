...
const BaseLayout: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setOpenedTabs((currentTabs) => {
        ...
        const mergedTabs = existingIndex >= 0
          ? currentTabs.map((item, index) => index === existingIndex ? { ...item, ...nextTab, pinned: item.pinned || nextTab.pinned } : item)
          : [...currentTabs, nextTab];
        const normalizedTabs = mergedTabs.some((item) => item.path === '/dashboard')
          ? mergedTabs
          : [{ path: '/dashboard', titleKey: 'dashboard.title', fallbackTitle: t('dashboard.title'), closable: false, pinned: true }, ...mergedTabs];
        const limitedTabs = limitOpenedTabs(normalizedTabs.map((item) => ({
          ...item,
          closable: item.path !== '/dashboard' && item.closable !== false,
          pinned: item.path === '/dashboard' || Boolean(item.pinned),
        })));
    ...
    const walk = (nodes: MenuNode[], ancestors: MenuNode[] = []) => {
      nodes.forEach((item) => {
        ...
        if (item.path && item.type !== 'F') {
          ...
          const parentTrail = trail.slice(0, -1).map((node) => t(node.titleKey)).join(' / ');
          items.push({
            key: `menu-${item.id}`,
            title,
  ...
  const noticeRiskGroups = useMemo<NoticeRiskItem[]>(() => {
    ...
    if ((isAdmin || hasPerm('system:operation-log:list')) && noticeSummary.todayOperationCount > 0) {
      groups.push({
        key: 'risk-operation',
  ...
  const closeTab = (targetPath: string) => {
    ...
    const safeTabs = nextTabs.length > 0
      ? nextTabs
      : [{ path: '/dashboard', titleKey: 'dashboard.title', fallbackTitle: t('dashboard.title'), closable: false, pinned: true }];
  ...
  const closeOtherTabs = (targetPath: string) => {
    ...
    const nextTabs = orderOpenedTabs(openedTabs.filter((item) => item.pinned || item.path === targetPath));
  ...
  const closeTabsToRight = (targetPath: string) => {
    ...
    const nextTabs = orderOpenedTabs(openedTabs.filter((item, index) => item.pinned || index <= targetIndex));
    localStorage.setItem(OPENED_TABS_STORAGE_KEY, JSON.stringify(nextTabs));
    setOpenedTabs(nextTabs);
  ...
  const togglePinTab = (targetPath: string) => {
    const nextTabs = orderOpenedTabs(openedTabs.map((item) => {
  ...
  return (
    <Layout className={['app-shell', isHorizontalLayout ? 'app-shell--horizontal' : 'app-shell--vertical'].join(' ')}>
      {!isHorizontalLayout ? (
        <Sider
      ...
      <Layout className="app-shell__main">
        ...
        <div className="app-shell__tabs" role="tablist" aria-label={t('app.openedTabs')}>
          {openedTabs.map((item) => {
            ...
            const canCloseOthers = openedTabs.some((tab) => tab.path !== item.path && tab.closable && !tab.pinned);
            const canCloseRight = openedTabs.slice(itemIndex + 1).some((tab) => tab.closable && !tab.pinned);
              ...
              >
                <div
                  ...
                  onDragOver={(event) => {
                    ...
                    if (!dragTab || dragTab.path === '/dashboard' || Boolean(dragTab.pinned) !== Boolean(item.pinned)) {
                      return;
                    }
                    event.preventDefault();
                ...
                >
                  {item.pinned ? (
                    <IconPushpin className="app-shell__tab-pin" />
                  ) : null}
                  <span className="app-shell__tab-label">{item.titleKey ? t(item.titleKey) : item.fallbackTitle}</span>
                  {canCloseCurrent ? (
                    <span
                      role="button"
                      tabIndex={0}
      ...
      >
        ...
        <div className="app-command__results">
          {filteredCommandItems.length > 0 ? filteredCommandItems.map((item, index) => {
