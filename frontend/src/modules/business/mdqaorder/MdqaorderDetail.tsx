...
import { getMdqaorderDetail, type MdqaorderDetail } from './api';
...
const governanceTableRole = 'main';
const governancePrimaryTable = '';
const governanceRelationFromField = '';
const governanceRelationToField = '';
...
const MdqaorderDetailPage: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const scope = 'business';
  const navigate = useNavigate();
  const { t } = useTranslation();
  const [detail, setDetail] = useState<MdqaorderDetail | null>(null);
  ...
  const loadDetail = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await getMdqaorderDetail(Number(id));
      setDetail(result);
    } catch (requestError) {
  ...
  const loadRelationData = useCallback(
    async (nextDetail: MdqaorderDetail) => {
      ...
      await Promise.all(
        governanceRelations.map(async (relation) => {
          ...
          try {
            if (relation.type === 'manyToMany' && !relation.lookupApi) {
            ...
            if (relation.type === 'lookup' && relation.lookupApi) {
              ...
              const localValue = nextDetail[relation.localField as keyof MdqaorderDetail];
  ...
  const submitChildRelationForm = useCallback(async () => {
    ...
    try {
      const payload = {
        ...values,
        [relation.targetField]: detail[relation.localField as keyof MdqaorderDetail],
      };
    ...
    <PageContainer>
      <Card bordered={false}>
        ...
        <Descriptions column={1} data={[
          { label: t('business.mdqaorder.field.name.label'), value: String(detail.name ?? '-') },
          { label: t('business.mdqaorder.field.status.label'), value: String(detail.status ?? '-') },
        ]} />
      </Card>
      <Card bordered={false}>
...
export default MdqaorderDetailPage;
