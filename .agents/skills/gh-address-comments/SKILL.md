...
description: Use when Pantheon Base has actionable GitHub PR review comments, issue comments, or discussion comments and the agent should fix, verify, and respond automatically with gh CLI
---
...
- Escalate only when feedback would:
  ...
  - conflict with repository contracts or ownership boundaries
...
- If a comment touches auth, permission, audit, i18n, generator, dynamic-module, import/export, middleware, or other runtime-sensitive paths, include the smallest relevant runtime or security evidence, not only unit tests.
- If a comment touches `.github/workflows/*`, reproduce with `repo-ci-triage` first and use `gh-fix-ci` if the hosted run still contains the real signal.
- If a comment proposes a change outside `pantheon-base` ownership, explain the boundary and route the work to the correct repository instead of applying a local workaround.
