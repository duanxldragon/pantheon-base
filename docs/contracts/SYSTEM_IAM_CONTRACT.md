...
- `system_user.preference_json.language` 归属长期用户偏好，不代表每次会话都必须强制覆盖当前登录语言

### 7.1.1 用户偏好与会话覆盖边界

- 用户语言偏好只在“当前会话没有显式语言覆盖”时参与运行时解析
- 登录页语言选择与系统内临时语言切换属于 `platform` 会话语义，不由 `system/iam` 强制落库
...
- `workbench` 首屏必须服务整改，而不是只提供全量体检表
- `workbench` 首屏必须支持按角色浏览完整授权结果，不能用默认过滤把 `clean` 角色直接隐藏
- 角色治理状态固定为 `pending | remediated | clean`
- `overview` 必须至少提供：
...
- 任何新权限模型变更，不得绕开 [PERMISSION_MODEL.md](D:/workspace/go/pantheon-platform/docs/designs/PERMISSION_MODEL.md)

### 7.6 批量治理交互约束

- 用户、角色、权限等治理列表的选择集绑定当前查询上下文，而不是绑定当前页
- 翻页不得清空选择集；搜索、筛选、排序、重置等改变查询上下文的动作必须清空选择集
- 批量治理动作必须基于完整选择集执行，当前页选中态只负责映射可见项
...
- 支持导出盘点与受控整改
- 整改后能够区分“仍待整改”“已整改”“无需处理”
- 最近整改动作能够通过 remediation events 回看
...
- 权限工作台的整改摘要、角色状态与 remediation events 时间线必须口径一致
