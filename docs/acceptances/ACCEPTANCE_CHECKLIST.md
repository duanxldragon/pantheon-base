...
- 详情加载是否正确
- 表单校验是否完整
- 若页面存在批量治理动作，是否验证“翻页不清空选择集、查询上下文变化才清空选择集”
- 批量删除、批量启停等动作是否基于完整选择集执行，而不是只基于当前页可见项
...
- 登录页显式选择语言后，登录成功进入系统是否保持同一语言
- 当前用户已有语言偏好时，是否不会反向覆盖本次登录页显式语言选择
- 系统内语言切换是否默认只影响当前会话，而不是自动写入长期偏好
- 登出后是否清理会话级语言覆盖，避免污染下一位登录主体
