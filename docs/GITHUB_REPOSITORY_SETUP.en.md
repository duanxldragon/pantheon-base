...
Do not add `Full Smoke Suite` or `Sonar` to required checks. `Quality Gates` must stay fast and deterministic for PR feedback, while `Full Smoke Suite` remains manual or release-precheck only. Keep Sonar and Codacy out of required checks. See [code quality and security strategy](./designs/QUALITY_AND_SECURITY_STRATEGY.md) for the gating model and thresholds.
