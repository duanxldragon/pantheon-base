...
后续继续深挖 `system/config` 时，又确认了一个容易误判为“系统设置故障”的权限边界：

1. **系统设置页属于“页面权限 + Casbin 接口权限”双轨页面**
2. **只给 `system:setting:list`，但未补 `GET /api/v1/system/setting/list` 等接口策略时，会出现“能进路由但页面主体加载失败”**
3. **系统设置页底部的配置变更审计属于 `system/audit` 能力，不应强制所有设置查看角色都去请求**

本次已同步收口：

- 设置页低权限场景下，未具备统一审计查看能力时不再默认拉取审计区块
- smoke 新增“只读角色可查看设置页，但保存/刷新缓存按钮禁用”的权限回归
- 文档补充 `system/config -> setting` 的双轨权限说明，避免角色误配
