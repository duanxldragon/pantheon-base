...
| `FR-002` | runtime-quality | ci-signal-noise | consumer-repository | 3 | PR quality work repeatedly chased broad smoke, workflow, Sonar, and duplication symptoms together. | Remediation time moved from code quality into CI stabilization and selector or fixture repair. | repo-quality-gate | `docs/designs/QUALITY_AND_SECURITY_STRATEGY.md` | `.github/workflows/quality.yml` | `Quality Gates`, `Duplication Gate` | workflow review | previous required-check docs | gate | gate-updated | none | implemented |
| `FR-003` | maintainability | static-sensor-gap | consumer-template | 3 | Sonar literal duplication and low-risk code-smell batches recurred across many commits. | AI output did not absorb maintainability constraints before CI or Sonar feedback. | repo-quality-gate | `docs/remediations/MAIN_SONAR_REMEDIATION_RUNBOOK_20260603.md` | SonarCloud, `npm run check:duplication` | duplication gate | Sonar remediation commits | task packet profile | template | template-updated | none | implemented |
...
- Repeated failures: CI signal noise, Sonar maintainability debt, security boundary drift, smoke scope drift, unrecorded ratchet events, cache hygiene, and sibling-path method wrappers are now tracked.
...
- Rules to remove or downgrade: Sonar and Codacy remain auxiliary; `Full Smoke Suite` stays scheduled, manual, or release-precheck.
