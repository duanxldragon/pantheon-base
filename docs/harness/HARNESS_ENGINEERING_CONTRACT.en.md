...
Pantheonâ€™s Harness Engineering goal is to make the repository itself the execution environment instead of locking process knowledge into a specific tool.
...
It is not enough to say â€œdoneâ€? â€œverifiedâ€? or â€œshould be fineâ€? The executor must provide:
...
The implementer may provide self-check notes, but â€œI checked it myselfâ€?is not the default completion decision.
...
Implementation must stay within the task packet scope. Do not opportunistically refactor unrelated code or â€œfixâ€?issues that have not been attributed.
