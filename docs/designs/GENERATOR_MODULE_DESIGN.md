...
本文定义 `system/generator` 的正式边界，并说明它如何挂接到 `platform.lowcode` 工作域。
...
- `metadata.autoRecycle`

它用于把临时 QA / smoke 模块显式标记为“彻底删除时自动回收托管数据表”，并由治理页展示为临时生命周期标签，而不是隐式依赖业务上下文推断。
...
- `metadata.autoRecycle` 只影响生成后的治理语义，不放宽任何高敏动作保护
...
- 已补：生成器前端显式暴露“临时模块 / 自动回收”开关，并把 `metadata.autoRecycle` 提交给治理链路。
...
`generator` 不应再被挂回 `system/config`，也不能被理解成“普通代码辅助页”。

它的正确认知应该是：

- `platform.lowcode` 工作域下的独立系统能力
- 能力归属为 `system/generator`
- 页面访问和高敏动作必须拆权
