...
- `/system/setting` 作为总览入口，只负责展示配置概览、风险摘要和分组导航
- `/system/setting/:groupKey` 作为分组页，只承载单一分组的表单编辑
- 按 `basic / platform / security / login / audit / upload / i18n / ui` 分组维护配置
- 敏感配置不回显明文，提供“留空保持不变”交互
- 配置变更审计只在 `/system/setting/audit` 分组页展示
- 审计项展示操作人、操作 IP、变更字段、状态、操作时间
- 敏感字段只显示“已变更”，不展示前后值
- 设置页不再提供页面级治理抽屉，避免与页签、面包屑和主操作区重复表达导航语义
...
- 仅当用户具备统一审计查看能力，且当前位于 `/system/setting/audit` 分组页时，才展示并拉取设置审计区块；
- 对只读设置角色，设置总览页和普通分组页应可正常加载，但不默认请求审计数据；
