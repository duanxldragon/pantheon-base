...
3.  **GitHub 原生质量门**: 所有非 trivial PR 默认只走 GitHub 原生门禁，不再依赖 Sonar 作为 required check。最低门槛：
    - `Quality Gates` 通过
    - `Security Gates` 通过
    - `Duplication Gate` 通过，且全库重复率 `<= 3%`
    - 低代码 smoke 结束后，generated registry、schema、临时业务模块目录和 generated i18n 资源必须被清理
...
7.  **评审输出必须留痕**: PR 描述至少记录归属层、改动边界、验证命令、GitHub checks 结果、Duplication Gate 结果、独立 reviewer 结论，以及是否存在挂账风险。
...
9.  **低代码产物必须清理**: 浏览器 smoke 与 generated business smoke 在执行前后都必须清理 generated modules、generated registry、schema/generated/business 和 generated i18n 资源；`business/cmdb` 仅作为烟测样板，不作为常驻业务代码。
...
2. 开启 Required Status Checks，并把 `Quality Gates`、`Security Gates`、`Duplication Gate` 设为必过检查
...
7. 统一使用 GitHub Checks 作为唯一合并口径，避免 reviewer 在多个系统间切换
