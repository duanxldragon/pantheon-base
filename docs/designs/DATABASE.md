...
- **默认值关系**: `system/config` 的公开设置继续提供默认主题和默认语言；运行时优先级先看当前会话覆盖，再看 `preference_json`，最后回退到系统默认值。
...
- **来源**: 表结构由权威迁移 `backend/pkg/database/migrations/*.sql` 创建；应用启动时通过 GORM Casbin Adapter 确保管理员默认策略。`database/system_init.sql`（旧 16 表 DDL，已 DEPRECATED）仅作历史参考，不再用于建库。
...
- **权威建库路径 = golang-migrate**：`docker-compose up` 由 `MYSQL_DATABASE` 创建空库，应用启动时自动执行 `migrate up`（29 表）并 seed（`backend/modules/system/seed.go` + i18n seed）。见 [database/README.md](../../database/README.md)。
- [旧 DDL 脚本（DEPRECATED，仅参考）](../../database/system_init.sql)——已落后于迁移，切勿用于建库。
