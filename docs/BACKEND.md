...
- **会话**: `system_user_session` 保存 refresh token 当前 JTI，刷新时轮换 JTI，注销时吊销会话。
...
- **审计**: `OperationLogMiddleware` 异步写入 `system_log_oper`，对 `password`、`token` 等敏感键递归脱敏，并支持业务模块通过上下文覆盖 `title / businessType / param / result`，让配置审计等场景复用统一操作日志底座。
...
| `GET` | `/api/v1/auth/security` | 是 | 获取当前登录账号安全概览，包含当前设备、活跃会话数、最近成功登录时间。 |
...
| `GET` | `/api/v1/system/menu/tree` | 是 | 获取动态菜单树；默认按 `scope=nav` 返回当前用户可见导航菜单树，并附带 `icon / routeName / module / isCache / isExternal / activeMenu` 元数据。 |
| `GET` | `/api/v1/system/menu/tree?titleKey=&path=&isVisible=&sortField=&sortOrder=&scope=manage` | 是 | 返回菜单管理页所需的全量菜单树，支持标题键、路径、显示状态筛选，以及树节点服务端排序。 |
...
| `GET` | `/api/v1/system/dept/tree` | 是 | 获取部门树，支持按部门名称、状态筛选及树节点排序；筛选命中子部门时会自动补齐祖先路径。 |
...
| `POST` | `/api/v1/system/dept/export` | 是 | 按当前筛选条件导出部门树为 CSV；部门使用 `path` 作为稳定引用。 |
...
| `POST` | `/api/v1/system/post/export` | 是 | 按当前筛选条件导出岗位 CSV。 |
...
| `GET` | `/api/v1/system/permission/workbench` | 是 | 获取 IAM 权限工作台视图，统一返回角色的导航授权、页面/按钮权限与 Casbin 接口策略。 |
...
| `GET` | `/api/v1/system/setting/group/:groupKey` | 是 | 按分组获取系统设置，当前已支持 `basic / security / login / upload / i18n / ui`。 |
...
- **权限策略**: 仅管理 `casbin_rule` 中的 `p` 策略，要求 `role_key` 存在，且 `(role_key, path, method)` 唯一。
...
- **联调参考**: 接口级手工验证步骤与示例 CSV 请见 `docs/SYSTEM_IMPORT_EXPORT_SMOKE_GUIDE.md`。
...
- **导航菜单**: `scope=nav` 只返回当前用户已授权、`is_visible=1`、且 `type <> 'F'` 的菜单；为保证树结构完整，会自动补齐其祖先节点。
- **管理菜单**: `scope=manage` 返回完整菜单树，供菜单管理页与角色授权页使用。
