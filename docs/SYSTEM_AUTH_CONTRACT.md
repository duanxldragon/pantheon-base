...
- 部门与岗位治理
- 业务域账号模型
- 在未确定身份源前实现真实 `MFA / CAPTCHA / SSO` 协议流
...
- [AUTH_MODULE_DESIGN.md](D:/workspace/go/pantheon-platform/docs/AUTH_MODULE_DESIGN.md)
- [SECURITY_CENTER_DESIGN.md](D:/workspace/go/pantheon-platform/docs/SECURITY_CENTER_DESIGN.md)
...
- `login.captcha_enabled / login.mfa_enabled / login.sso_enabled` 可以作为配置开关存在
- 但未进入真实协议实现前，不展示伪能力入口
...
- 密码最小长度、登录失败锁定、来源/IP 节流、空闲超时、最大会话数有统一策略入口
...
- [PLATFORM_AUTH_REMEDIATION_CLOSEOUT_20260429.md](D:/workspace/go/pantheon-platform/docs/PLATFORM_AUTH_REMEDIATION_CLOSEOUT_20260429.md)
