...
const (
	...
	I18nSortLocaleASC    = "locale ASC"
	I18nSortGroupNameASC = "group_name ASC"
	I18nWhereModule      = "module = ?"
)

func (s *I18nService) ReloadCache() error {
	var items []SystemI18n
	if err := s.db.Order(I18nSortLocaleASC).Order(I18nSortModuleASC).Order(I18nSortKeyASC).Find(&items).Error; err != nil {
		return err
	}
	s.mu.Lock()
	defer s.mu.Unlock()
