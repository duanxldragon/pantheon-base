...
func collectSystemInfo(client *ssh.Client) (*systemInfo, error) {
	...
	output, err := session.CombinedOutput(
		"uname -s && uname -r && nproc && " +
			...
			`df -BG --total 2>/dev/null | tail -1 | awk '{print $2}' | sed 's/G//' || echo 0`,
	)
	if err != nil {
		return nil, err
	}
	...
	if len(lines) >= 5 {
		info.OS = strings.ToLower(lines[0])
		info.OSVersion = lines[1]
		info.CPUCores, _ = strconv.Atoi(lines[2])
		info.MemoryGB, _ = strconv.ParseFloat(lines[3], 64)
		info.DiskGB, _ = strconv.ParseFloat(lines[4], 64)
	}
	if info.OS == "" {
		info.OS = "linux"
	}
