...
func (s *MdqaorderitemService) Migrate() error {
	...
	if err := s.db.AutoMigrate(&Mdqaorderitem{}); err != nil {
		return err
	}
	return nil
}
...
// ListMdqaorderitemOptions 关系选择器选项
func (s *MdqaorderitemService) ListMdqaorderitemOptions() ([]MdqaorderitemOptionItem, error) {
	var items []Mdqaorderitem
	if err := s.db.Model(&Mdqaorderitem{}).Order("id desc").Limit(100).Find(&items).Error; err != nil {
		return nil, err
	}
	options := make([]MdqaorderitemOptionItem, len(items))
	for i, item := range items {
		options[i] = MdqaorderitemOptionItem{
			Label: item.ItemName,
			Value: item.ID,
			ID:    item.ID,
			Name:  item.ItemName,
