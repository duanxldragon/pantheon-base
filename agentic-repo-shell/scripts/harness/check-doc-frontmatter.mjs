...
const ALLOWED_DOC_TYPES = new Set(['Contract', 'Design', 'Assessment', 'Remediation', 'Acceptance']);
...
export function parseFrontmatter(source) {
  ...
  if (lines[0] !== '---') return { data: null, body: source, hasFrontmatter: false };
  const closingIndex = lines.findIndex((line, index) => index > 0 && line === '---');
  if (closingIndex === -1) return { data: null, body: source, hasFrontmatter: false };
...
function isPlaceholderRef(ref) {
  return ref === 'TBD' || ref.endsWith('/TBD.md') || ref.endsWith('\\TBD.md');
}
...
function buildDocsIndex(repoRoot) {
  ...
  for (const filePath of files) {
    ...
    const entry = { path: relativePath, basename: path.basename(relativePath), source, parsed };
...
export function extractReadmeMainEntryLinks(source) {
  ...
  for (const line of lines) {
    if (/^## 2\./.test(line) || /^## 3\./.test(line) || /^## 4\./.test(line)) inMainEntry = true;
    else if (/^## 5\./.test(line) || /^## 6\./.test(line)) inMainEntry = false;
    if (!inMainEntry) continue;
    for (const href of extractReadmeDocLinks(line)) captured.push(href);
  }
  return captured;
}
...
export function validateDoc({ filePath, data, repoRoot }) {
  ...
  for (const field of REQUIRED_BASE_FIELDS) {
    if (!isNonEmptyString(data[field])) errors.push(`${filePath}: missing or empty required field "${field}"`);
  }
  ...
  if (expectedGroup) {
    for (const field of REQUIRED_RETAINED_FIELDS) {
      if (field === 'linked_contracts') {
        if (!isNonEmptyArray(data[field])) errors.push(`${filePath}: missing or empty required field "${field}"`);
  ...
  if (data.linked_contracts !== undefined) {
    if (!Array.isArray(data.linked_contracts)) {
      ...
      for (const linkedPath of data.linked_contracts) {
        ...
        if (!fs.existsSync(absolute)) errors.push(`${filePath}: linked contract path does not exist: ${linkedPath}`);
      }
    }
  }
...
function checkFile(filePath, repoRoot) {
  ...
  try {
    ...
    return { ok: false, errors: [`${relativePath}: failed to parse frontmatter: ${error.message}`] };
  }
  if (!parsed.hasFrontmatter) {
    const expectedGroup = expectedIndexGroup(relativePath);
    if (expectedGroup) return { ok: false, errors: [`${relativePath}: missing YAML frontmatter block`], legacy: hasLegacyMetadata(source) };
    return { ok: true, errors: [], legacy: hasLegacyMetadata(source) };
  }
  return { ...validateDoc({ filePath: relativePath, data: parsed.data ?? {}, repoRoot }), legacy: false };
}

export function runCheck(repoRoot = DEFAULT_REPO_ROOT) {
  const docsIndex = buildDocsIndex(repoRoot);
  const errors = [];
  const legacyFiles = [];
  let frontmatterFiles = 0;

  for (const filePath of docsIndex.files) {
    const result = checkFile(filePath, repoRoot);
    const relativePath = normalizeSlash(path.relative(repoRoot, filePath));
    const parsed = docsIndex.byPath.get(relativePath)?.parsed;
    if (parsed?.hasFrontmatter) frontmatterFiles += 1;
    if (!result.ok) errors.push(...result.errors);
    if (result.legacy) legacyFiles.push(relativePath);
  }
  ...
  for (const [relativePath, entry] of docsIndex.byPath.entries()) {
    ...
    for (const { field } of CONTRACT_RELATION_FIELDS) {
      const bodyRefs = relationRefs.byField[field].filter((ref) => !isPlaceholderRef(ref));
      const fmRefs = (Array.isArray(entry.parsed.data?.[field]) ? entry.parsed.data[field] : []).filter(
        (ref) => !isPlaceholderRef(ref),
      );
      const resolvedBody = bodyRefs.map((ref) => resolveContractBodyReference(ref, docsIndex)?.path ?? null).filter(Boolean).sort();
      const resolvedFm = fmRefs.map((ref) => resolveContractBodyReference(ref, docsIndex)?.path ?? ref).filter(Boolean).sort();
  ...
  return { ok: errors.length === 0, checkedFiles: docsIndex.files.length, frontmatterFiles, legacyFiles, errors };
}
...
if (isDirectRun) {
  ...
  if (!result.ok) {
    ...
    if (showLegacy && result.legacyFiles.length > 0) {
      ...
      for (const file of result.legacyFiles) console.error(`- ${file}`);
    }
    process.exit(1);
  }
  ...
  if (showLegacy) {
    ...
    for (const file of result.legacyFiles) console.log(`- ${file}`);
  }
}
