...
- **真相源单一**：跨模块边界、权限命名、视觉 token、响应式断点都必须有唯一 canonical 文档，避免多份规范并存后各写各的。
...
### 2.4 Feature Ledger 与演进回路

- 所有生成能力都必须有机器可读的 Feature Ledger 条目，不能只靠口头约定判断“这是不是平台能力”。
- 生成能力的真相源是 `system_module_registration` + `schema/generated/**.json`；派生快照落在 `schema/generated/feature-ledger.json`。
- `feature-ledger.json` 是派生产物，不是第二份手工注册表，也不能被模块扫描逻辑重新当作业务模块。
- Ledger 的作用是把“为什么存在、是否通用、是否成熟”转成可校验的投影，让演进回路从文档走向机器检查。
- 角色语义映射到本仓库时，`Product / Architect / Builder / Auditor / Evolution` 分别对应 `Planner / Explorer / Executor / Reviewer / ratchet`。
- 重复漂移必须沉淀为 checker、guide、gate、template 或 adapter 的升级，而不是保留在聊天记录里。
...
**筛选区统一使用 `SearchToolbar`**（`frontend/src/components/patterns/SearchToolbar.tsx`）：
左侧防抖关键词框（placeholder 必须走 i18n）+ 行内高频下拉（变更即查询）+
低频筛选收进"筛选"弹层（活跃计数徽标）+ 有活跃筛选时的"清空筛选"入口；
窄屏（≤768px）行内下拉自动并入弹层。FilterPanel 表单栅格（显式搜索/重置按钮）
已整体退役。工具栏内输入类控件必须继承全局 `.arco-*` 控件规则（不得重声明
border/background，避免交互态被高特异性规则压掉）。机械门禁：
`npm run check:search-toolbar-contract`（prebuild 链强制执行）。
...
| 角色               | 字体           | 说明                                                                                                                                          |
| :----------------- | :------------- | :-------------------------------------------------------------------------------------------------------------------------------------------- |
| **UI 正文 / 全局** | system-ui 栈   | 以 `system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Microsoft YaHei', sans-serif` 为准，随系统中性无衬线，零外部依赖               |
| **代码 / 数据**    | JetBrains Mono | 等宽，连字支持，运维日志与代码编辑器场景（`CodePreview` 局部声明）                                                                            |
| **可选增强**       | Source Sans 3  | Adobe 开源 UI 字体，支持 tabular-nums。**当前实现未加载**；如需引入，自托管 woff2 或 Google Fonts 均可，置于 system-ui 栈之前即可，非强制契约 |

字体真相源为 `frontend/src/index.css` 的 `font-family`，当前使用 system-ui 栈。`body` 已开启 `font-feature-settings: 'tnum'` 保证表格数字等宽。Source Sans 3 为可选增强而非强制项——避免在内网 / 离线部署中因外部字体缺失导致回退不可控。
...
**间距基准：** 使用实现中的完整 token 阶梯；2px 只用于极紧密的内部微间距，常规布局从 4px 起。

| Token          | 值   | 典型用途                   |
| :------------- | :--- | :------------------------- |
| `--space-2xs`  | 2px  | 紧密文本或图标内部微间距   |
| `--space-xs`   | 4px  | 表内标签、紧密图标间距     |
| `--space-sm`   | 8px  | 同类控件与紧凑表单间距     |
| `--space-md`   | 12px | 工具栏、表单行与局部区段   |
| `--space-lg`   | 16px | 卡片内边距、标准区段间距   |
| `--space-xl`   | 24px | 页面内容区 padding         |
| `--space-2xl`  | 32px | 大区块间隔                 |
| `--space-3xl`  | 48px | 页面级分区或宽松留白       |
...
| `--radius-sm`      | 4px                | 紧凑按钮、动作入口               |
| `--radius-md`      | 6px                | 输入控件、卡片与常规面板         |
| `--radius-lg`      | 8px                | 较大面板和分组表面               |
| `--radius-xl`      | 12px               | 需要更强分组感的大型表面         |
| `--radius-overlay` | 8px                | 弹窗、抽屉等浮层                 |
| `--radius-control` | `var(--radius-md)` | 输入框、选择器等控件             |
| `--radius-action`  | `var(--radius-sm)` | 按钮、分页与动作控件             |
| `--radius-pill`    | 999px              | 胶囊标签、状态指示点             |
...
- Inter 作为主字体（现以 system-ui 栈为准；Source Sans 3 仅作可选增强）
- FilterPanel 表单栅格筛选区（显式搜索/重置按钮）重新引入列表页（由 `check:search-toolbar-contract` 拦截）

所有视觉表面使用纯色 Token。层次通过边框、弱阴影、间距来建立。

本清单由机械门禁强制执行：`frontend/scripts/check-ui-contract.mjs`（全库扫描 §7.6/§7.7/§7.9 违规，挂在 `prebuild` 链）+ `frontend/scripts/check-shell-visual-contract.mjs`（壳层结构断言）。单行豁免使用 `ui-contract-allow: <rule-id>` 行内注释并在 PR 说明理由。
